...
`--max-jobs-per-second 1 --max-status-checks-per-second 0.01` - used to not overwhelm job manager  
`-s` - indicate snakefile name

Below is an example of grouping 288 samples in batches of 24 for SLURM runs using 4 cpus, 4G of memory, and 30 minutes ([snakemake details on cluster integration](https://snakemake.readthedocs.io/en/stable/executing/cluster.html): 
```
...
```
The 'align' group runs the following rules for each sample: 
- `trim`: cutadapt trimming of reads
- `bbmap_map`: aligning trimmed reads with bbmap
- `sort`: filter, sort, and index alignment
- `rmdup`: remove duplicate reads
- `pull split`: pull split reads output txt file
- `divrge`: run divrge on split reads
- `ghost_reads`: calculate read depth across rmdup alignment
...
params:  # general and alignment parameters
  ...
  primers: path and filename to adapters and primers used for trimming (ex: "/share/apps/trimmomatic/0.36/adapters/NexteraPE-PE.fa")
  rev_primers: path and filename to a fasta with the reverse comp of adapters and/or primers used for trimming.
...
divrge_param: # divrge specific parameters
  mismatch: number of mismatches "X" allowed in read alignment (default: "2")
  indel: number of indels ("D"/"I") allowed in the read alignment (default: "2")
  ...
  nbam: number of samples input (default: "1") - for grouping of DVGs output by more than one sample, increase #
  gap_type: "N" cigar string for gap 
  min_count: min. number of reads used for filtering and cat rule
  qual_cutoff: quality requirement for read depth calcs (default: 25)
...
```
params:
  bamfiles: full directory path to alignments you want to use as input into DiVRGE specific scripts (ex: "/path/to/alignment/files/bamfiles/rmdups")
  begin: string that proceeds the name for files likely empty ""
  ending: string that follows the sample name (ex: ".H9N2.rmd.bbmap.bam", ".bam", ".sorted.bam")
```
...
- `metric`: metric files when running picard MarkDuplicates
...
- `read_depth`: read depth files used for calculating frequency
